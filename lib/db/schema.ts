...
export const user = pgTable(
  "user",
  {
    ...
    pinHash: varchar("pin_hash", { length: 120 }).notNull(),
    createdAt: timestamp("created_at").notNull().defaultNow(),
    updatedAt: timestamp("updated_at").notNull().defaultNow(),
  },
  (table) => ({
...
export const patientProfile = pgTable(
  ...
  (table) => ({
    profileUserIdx: index("idx_patient_profile_user_v1").on(table.userId),
    profileClinicIdx: index("idx_patient_profile_clinic_v1").on(table.clinicId),
  })
);
